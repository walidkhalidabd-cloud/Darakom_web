...
const [formData, setFormData] = useState({
    first_name: '', last_name: '', email: '', phone: '', province_id: '', address: '', bio: ''
  });
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        // في حال عدم توفر الباك، نستخدم بيانات افتراضية
        setFormData({
          first_name: 'أحمد', last_name: 'سليمان', email: 'ahmed.s@example.com',
          phone: '0999123456', province_id: '', address: 'دمشق',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showToast('error', '⚠️ تعذر الاتصال بالخادم، تم حفظ البيانات محلياً.');
...
{/* حقل المحافظة كقائمة منسدلة */}
                <div className="col-md-12">
                  ...
                  <select className="form-control form-control-custom"
                    ...
                    {syrianGovernorates.map((gov, idx) => (
                      <option key={gov} value={idx + 1}>{gov}</option>
                    ))}
                  </select>
                </div>
