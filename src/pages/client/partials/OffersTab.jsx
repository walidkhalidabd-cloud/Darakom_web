...
import { fetchClientProjects, fetchClientProjectOffers, acceptOffer, rejectOffer, deleteProject } from '../../../services/api/clientApi';
...
const [offerStatus, setOfferStatus] = useState('pending');
    ...
    // تحويل بيانات الباك إلى هيكل الواجهة
    const mapProjects = (data) => data.map(p => {
        const status = p.execution_status || p.status || 'new';
        const progress = p.progress_percentage || 0;
        const offersCount = p.offers?.length || 0;
        return {
            id: p.id,
            projectTitle: p.title,
            providerType: p.work_type === 'construction' ? 'إنشاء' : 'تشطيب',
            price: p.budget ? String(p.budget) : '0 - بانتظار العروض',
            duration: p.tender_duration ? `${p.tender_duration} ${p.tender_duration_unit === 'hour' ? 'ساعة' : 'يوم'}` : '',
            datePosted: p.created_at ? p.created_at.slice(0, 10) : '',
            details: p.description,
            status: status === 'finished' ? 'مكتمل' : status === 'in_progress' ? 'قيد التنفيذ' : 'بانتظار العروض',
            progress,
            offersCount,
            daysRemaining: 0,
            governorate: p.province?.name || '',
            area: p.area || '',
            providerTypeNeeded: p.work_type,
            tenderDays: p.tender_duration || '',
            description: p.description,
            providerName: p.performer?.user?.full_name || '',
            provider: p.performer?.user || null,
            offers: p.offers || []
        };
    });

    // جلب مشاريع العميل من الباك (لإعادة التحميل بعد الحذف)
    const loadProjects = async () => {
        try {
            const res = await fetchClientProjects();
            const data = res.data?.data || [];
            setProjectsList(mapProjects(data));
        } catch {
            // في حال فشل الاتصال، نستخدم البيانات الوهمية افتراضية
            setProjectsList([]);
        }
    };
    ...
    useEffect(() => {
        ...
        (async () => {
            const res = await fetchClientProjects();
            const data = res.data?.data || [];
            if (!cancelled) setProjectsList(mapProjects(data));
        })().catch(() => {
...
// استخدام بيانات الباك إذا توفرت، وإلا نستخدم البيانات الوهمية كـ fallback
    const effectiveProjectsList = projectsList.length > 0 ? projectsList : null;
    const projects = effectiveProjectsList
        ? effectiveProjectsList.filter(p => {
            const isCompleted = p.progress === 100;
            const isOngoing = p.progress > 0 && p.progress < 100;
            if (offerStatus === 'pending') return !isOngoing && !isCompleted;
            if (offerStatus === 'ongoing') return isOngoing;
            return isCompleted;
        })
        : (offerStatus === 'pending' ? pendingOffers 
...
const viewProjectOffers = async (project) => {
        ...
        try {
            const res = await fetchClientProjectOffers(project.id);
            const offers = res.data?.data || [];
            // تحويل بيانات العروض من الباك إلى هيكل الواجهة
            const mapped = offers.map(o => ({
                id: o.id,
                providerName: o.provider?.user?.full_name || o.provider?.user?.name || 'مزود خدمة',
                providerType: o.provider?.role?.name || 'مزود خدمة',
                rating: o.provider_rating || 0,
                price: o.cost ? String(o.cost) : '',
            ...
            setProjectOffers(mapped.length ? mapped : (receivedOffersData[project.id] || []));
        } catch {
    ...
    const handleAcceptOffer = async (offer) => {
        ...
        try {
            await acceptOffer(selectedProject.id, offer.id);
            alert('تم قبول العرض بنجاح!');
            setProjectOffers(prev => prev.map(o => o.id === offer.id ? { ...o, status: 'accepted' } : o));
        } catch {
            alert('تعذر قبول العرض، يرجى المحاولة مجدداً.');
        }
    };
    ...
    const handleRejectOffer = async (offer) => {
        if (!selectedProject) return;
        try {
            await rejectOffer(selectedProject.id, offer.id);
            alert('تم رفض العرض.');
            setProjectOffers(prev => prev.map(o => o.id === offer.id ? { ...o, status: 'rejected' } : o));
        } catch {
            alert('تعذر رفض العرض، يرجى المحاولة مجدداً.');
        }
    };
    ...
    const handleDeleteProject = async (projectId) => {
        ...
        try {
            ...
            alert('تعذر حذف المشروع، يرجى المحاولة مجدداً.');
        }
    };
...
{offer.status === 'pending' && (
                                            <>
                                                <button className="btn btn-success fw-bold px-4 py-2 rounded-pill shadow-sm" style={{ fontSize: '16px' }} onClick={() => handleAcceptOffer(offer)}>
                                                    <FaCheckCircle className="me-1" /> قبول العرض
                                                </button>
                                                <button className="btn btn-outline-danger fw-bold px-4 py-2 rounded-pill" style={{ fontSize: '16px' }} onClick={() => handleRejectOffer(offer)}>
