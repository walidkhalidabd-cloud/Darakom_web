...
const MainNavBar = () => {
    ...
    // توجيه المستخدم للوحة التحكم الخاصة به حسب نوع الحساب
    const getDashboardPath = () => {
        if (user.type === 'admin') return '/admin/dashboard';
        if (user.type === 'provider') return '/provider/dashboard';
                    ...
                    <div className="d-flex align-items-center gap-3">
                        {!isLogged ? (
                            ...
                            <div className="d-flex align-items-center gap-4">
                                ...
                                <Link to={getDashboardPath()} className="d-flex align-items-center gap-2 text-white text-decoration-none" style={{ cursor: 'pointer' }}>
                                    <FiUser className="text-white" style={{ fontSize: '35px' }} />
                                    <span className="fw-bold" style={{ fontSize: '20px' }}>{user.name?.split(' ')[0] || 'حسابي'}</span>
                                </Link>
                            </div>
                        )}
                    </div>
